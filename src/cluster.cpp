...
void writeClusteredPDB(const std::vector<std::vector<Atom>>& clusters, const std::string& filename) {
    // 1. Open File
    // Added ".pdb" extension handling in case filename doesn't have it, 
    // but your logic (filename + ".pdb") works if filename is just the stem.
    ...
    for (int i = 0; i < (int)clusters.size(); ++i) {
        ...
        for (const auto& atom : clusters[i]) {
            ...
            // Format Changes:
            // 1. Replaced "0.00" with "%6.2f" in the format string (Cols 61-66).
            // 2. Added atom.get_bfactor() to the argument list.
            ...
            fprintf(pFile, "HETATM%5d  %-3s %3s A%4d    %8.3f%8.3f%8.3f  1.00%6.2f           %s\n",
                    ...
                    resID,
                    c[0], c[1], c[2],
                    atom.get_bfactor(), // <--- NEW: Output the B-factor
